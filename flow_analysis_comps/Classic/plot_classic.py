...
def plot_fields(fourier_images:np.ndarray, speed_images:np.ndarray, video_deltas:videoDeltas):
    kymo_extent = [
        0,
    ...
    leftIm = ax["speedLeft"].imshow(speed_images[0], vmin=-speed_abs, vmax= speed_abs, cmap="cet_CET_D1A", extent=kymo_extent)
    rightIm = ax["speedRight"].imshow(speed_images[1], vmin=-speed_abs, vmax= speed_abs, cmap="cet_CET_D1A", extent=kymo_extent)
...
def plot_summary(fourier_images, speed_images, video_deltas:videoDeltas):
    ...
    speed_histo = (speed_histo_left + speed_histo_right) / (
        2 * len(speed_images[0][0])
    ) 

    
    fig, ax = plt.subplot_mosaic(
        [
            ["kymograph", "temporal histogram"],
            ["speed plot", "temporal histogram"]
        ]
        , layout="constrained"
    )

    ax["kymograph"].imshow(fourier_images[0] + fourier_images[1], cmap="cet_CET_L20")

    ax["speed plot"].plot(np.nanmean(speed_images[0], axis=1), c='tab:orange', label="speed left")
    # ax["speed plot"].fill_between()

    ax["speed plot"].plot(np.nanmean(speed_images[1], axis=1), c='tab:blue', label="speed right")
    ax["speed plot"].axhline(0, linestyle="--", c="black")
    ax["speed plot"].legend()

    ax["temporal histogram"].imshow(speed_histo.T,            
