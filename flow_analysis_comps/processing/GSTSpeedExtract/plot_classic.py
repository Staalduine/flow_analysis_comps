...
def plot_fields(
    fourier_images: kymoOutputs, speed_images: np.ndarray
):
    ...
    ax["fourierLeft"].imshow(fourier_images.kymo_left, cmap="cet_CET_L20", extent=kymo_extent)
    ax["fourierRight"].imshow(fourier_images.kymo_right, cmap="cet_CET_L20", extent=kymo_extent)
...
def plot_summary(fourier_images: kymoOutputs, speed_images: np.ndarray, name:str):
    ...
    time_axis_points = np.linspace(0, fourier_images.deltas.delta_t * len(fourier_images.kymo_left), len(fourier_images.kymo_left))
    ...
    ax["kymograph"].imshow(
        fourier_images.kymo_left + fourier_images.kymo_right, cmap="cet_CET_L20", extent=kymo_extent
    )
    ...
    ax["temporal histogram"].imshow(
        speed_histo.T,
        extent=(0, len(speed_histo) * fourier_images.deltas.delta_t, -speedmax, speedmax),
        origin="lower",
        cmap="cet_CET_L16"
    )
